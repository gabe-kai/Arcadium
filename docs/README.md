# Documentation

Project documentation for Arcadium.

## Structure

- `architecture/` - Architecture decisions and diagrams
- `api/` - API documentation
- `game-design/` - Game design documents

## Service Documentation

### Wiki Service
- [Wiki Service Specification](wiki-service-specification.md) - Complete requirements and features
- [Wiki Implementation Guide](wiki-implementation-guide.md) - Step-by-step implementation with testing
- [Wiki API Documentation](api/wiki-api.md) - REST API endpoints and schemas
- [Wiki Architecture](architecture/wiki-architecture.md) - System architecture and data flow
- [Wiki User Interface](wiki-user-interface.md) - UI/UX design and user flows
- [Wiki Version History](wiki-version-history.md) - Version control and rollback system
- [Wiki Orphanage System](wiki-orphanage-system.md) - Orphaned page management
- [Wiki Admin Dashboard](wiki-admin-dashboard.md) - Admin configuration and monitoring
- [Wiki Section Extraction](wiki-section-extraction.md) - Section extraction features
- [Wiki AI Content Management](wiki-ai-content-management.md) - AI agent writing workflow
- [Wiki Service Status Page](wiki-service-status-page.md) - Service health monitoring page
- [Wiki Open Questions](wiki-open-questions.md) - All questions resolved

### Auth Service
- [Auth Service Specification](services/auth-service.md) - Authentication and authorization service
- [Auth Service API](api/auth-api.md) - REST API endpoints
- [Auth Service Implementation Guide](auth-service-implementation-guide.md) - Step-by-step implementation plan
- [Auth Implementation Assessment](auth-implementation-assessment.md) - Design readiness assessment

### Notification Service
- [Notification Service Specification](services/notification-service.md) - Internal messaging and notifications
- [Notification Service API](api/notification-api.md) - REST API endpoints
