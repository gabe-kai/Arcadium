...
def create_database_and_schema(db_url, db_name="auth"):
    ...
    if not db_url.startswith("postgresql://"):
        print("Error: Invalid database URL format. Must start with 'postgresql://'")
        return False
...
def main():
    ...
    parser.add_argument(
        "--db-url", type=str, help="Base database URL (without database name)"
    )
    parser.add_argument(
        "--db-name",
        type=str,
        default="arcadium_auth",
        help="Database name (default: arcadium_auth)",
    )
    ...
    if args.db_url:
        ...
        if not db_url:
            ...
            if db_user and db_pass:
                ...
                print(
                    "Error: DATABASE_URL or (arcadium_user and arcadium_pass) must be set"
                )
                print(
                    "Set DATABASE_URL or set arcadium_user and arcadium_pass environment variables"
                )
                sys.exit(1)
    ...
    if not success:
        ...
        print("  4. Create schema: CREATE SCHEMA auth;")
        sys.exit(1)
