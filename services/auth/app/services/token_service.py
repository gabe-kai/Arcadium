...
class TokenService:
    ...
    def generate_access_token(user: User) -> str:
        ...
        expires_in = current_app.config.get('JWT_ACCESS_TOKEN_EXPIRATION', 3600)  # Default 1 hour
    ...
    def is_token_blacklisted(token_id: str) -> bool:
        ...
        """
        blacklisted = db.session.query(TokenBlacklist).filter_by(
            token_id=token_id
        ).first()
    ...
    def blacklist_token(token_id: str, user_id: str, expires_at: datetime):
        ...
        existing = db.session.query(TokenBlacklist).filter_by(
            token_id=token_id
        ).first()
