...
class PageService:
    ...
    def create_page(
        ...
        try:
            ...
            db.session.rollback()
            raise ValueError("Failed to create page: slug may already exist")

        # Get page_id after commit using raw SQL (avoid SQLite UUID issues)
        try:
            result = db.session.execute(
                db.text("SELECT id FROM pages WHERE slug = :slug"), {"slug": slug}
            ).first()
            if result:
                ...
                stored_page_id = (
                    uuid.UUID(result[0]) if isinstance(result[0], str) else result[0]
                )
            else:
                raise ValueError(f"Could not retrieve page after creation: {slug}")
        except Exception as e:
            raise ValueError(
                f"Could not retrieve page ID after creation: {slug} - {str(e)}"
            )
    ...
    def delete_page(page_id: uuid.UUID, user_id: uuid.UUID) -> Dict:
        ...
        page_info = {"id": str(page_id), "title": page.title, "slug": page.slug}

        # Store children info before deletion
        for child in children:
            orphaned_pages.append(
                {"id": str(child.id), "title": child.title, "slug": child.slug}
            )
        ...
        if children:
            ...
            orphaned_page_objects = OrphanageService.orphan_pages(
                    ...
                    if user_id:
                        ...
                        query = query.filter(
                            or_(
                                ...
                                and_(
                                    Page.status == "draft", Page.created_by == user_id
                                ),
                            )
                        )
                    else:
    ...
    def _build_file_content(
        page: Page, frontmatter: Dict, markdown_content: str
    ) -> str:
    ...
    def _create_version(
        ...
        prev_version = (
            PageVersion.query.filter_by(page_id=page_id)
        ...
        if page_title is None or page_content is None or page_version is None:
            ...
            try:
                ...
                if page_slug:
                    result = db.session.execute(
                        db.text(
                            "SELECT title, content, version FROM pages WHERE slug = :slug"
                        ),
                        {"slug": page_slug},
                    ).first()
                else:
                    result = db.session.execute(
                        db.text(
                            "SELECT title, content, version FROM pages WHERE id = :page_id"
                        ),
                        {"page_id": str(page_id)},
                    ).first()
                ...
                if result:
                    ...
                    raise ValueError(
                        f"Page not found in database (id: {page_id}, slug: {page_slug})"
                    )
            except Exception as e:
                ...
                try:
                    ...
                    raise ValueError(
                        f"Could not access page attributes: {page_id} - {str(e)}"
                    )
