...
class CacheService:
    ...
    def _get_cache_ttl() -> int:
        ...
        config = (
            ...
            .filter_by(key=CacheService.CACHE_TTL_KEY)
            .first()
        )
    ...
    def set_html_cache(content: str, html: str, user_id: Optional[str] = None):
        ...
        """
        cache_key = CacheService._generate_cache_key(content, "html")
        cache_data = {"html": html, "cached_at": datetime.now(timezone.utc).isoformat()}
        ...
        if config:
            ...
            config = WikiConfig(
                key=cache_key, value=json.dumps(cache_data), updated_by=cache_user_id
            )
    ...
    def set_toc_cache(content: str, toc: List[Dict], user_id: Optional[str] = None):
        ...
        """
        cache_key = CacheService._generate_cache_key(content, "toc")
        cache_data = {"toc": toc, "cached_at": datetime.now(timezone.utc).isoformat()}
