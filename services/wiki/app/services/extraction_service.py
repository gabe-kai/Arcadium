...
from app.utils.toc_service import generate_toc, _generate_anchor
...
class ExtractionService:
    ...
    def extract_selection(
        ...
        if selection_start < 0 or selection_end > content_length or selection_start >= selection_end:
        ...
        if replace_with_link:
            ...
            link_text = ExtractionService._extract_link_text(extracted_content, new_title)
        ...
        PageService.update_page(
            page_id=new_page.id,
            user_id=user_id,
            content=new_page_content_with_backlink
        )
    ...
    def _find_section_end(content: str, heading_start: int, heading_level: int) -> int:
        ...
        pattern = re.compile(r'^(#{2,' + str(heading_level) + r'})\s+(.+)$', re.MULTILINE)
