...
class CommentService:
    ...
    def get_comments(page_id: uuid.UUID, include_replies: bool = True) -> List[Dict]:
        ...
        if include_replies:
            ...
            top_level_comments = (
                db.session.query(Comment)
                .filter_by(page_id=page_id, parent_comment_id=None)
                .order_by(Comment.created_at.asc())
                .all()
            )
    ...
    @staticmethod
    def _build_replies_tree(
        parent_comment_id: uuid.UUID, comments_by_parent: dict
    ) -> List[Dict]:
        """
        ...
        for reply in replies:
            ...
            reply_dict["replies"] = CommentService._build_replies_tree(
                reply.id, comments_by_parent
            )
    ...
    @staticmethod
    def delete_comment(
        comment_id: uuid.UUID, user_id: uuid.UUID, user_role: str = "viewer"
    ) -> None:
        """
        Delete a comment.
