...
class VersionService:
    ...
    @staticmethod
    def create_version(
        page_id: uuid.UUID, user_id: uuid.UUID, change_summary: str = None
    ) -> PageVersion:
        """
        ...
        if latest_version:
            diff_data = VersionService._calculate_diff(
                latest_version.content, page.content
            )
    ...
    def get_version(page_id: uuid.UUID, version: int) -> Optional[PageVersion]:
        ...
        return (
            db.session.query(PageVersion)
            .filter_by(page_id=page_id, version=version)
            .first()
        )
    ...
    @staticmethod
    def compare_versions(page_id: uuid.UUID, version1: int, version2: int) -> Dict:
        """
        ...
        return {"version1": v1.to_dict(), "version2": v2.to_dict(), "diff": diff_data}

    @staticmethod
    def rollback_to_version(
        page_id: uuid.UUID, version: int, user_id: uuid.UUID, user_role: str = "viewer"
    ) -> Page:
        """
            ...
            summaries.append(
                {
                    ...
                    "diff_stats": (
                        version.diff_data.get("added_lines", 0)
                        + version.diff_data.get("removed_lines", 0)
    ...
    @staticmethod
    def get_version_diff(page_id: uuid.UUID, version1: int, version2: int) -> Dict:
        """
