...
class OrphanageService:
    ...
    def get_or_create_orphanage(user_id: uuid.UUID) -> Page:
        ...
        orphanage = (
            ...
            .filter_by(slug=OrphanageService.ORPHANAGE_SLUG, is_system_page=True)
            .first()
        )
        ...
        try:
            ...
            db.session.execute(
                db.text(
                    "UPDATE pages SET is_system_page = :is_system WHERE id = :page_id"
                ),
            ...
            orphanage = db.session.query(Page).filter_by(slug=orphanage_slug).first()
        except Exception:
            ...
            try:
                ...
                orphanage = (
                    db.session.query(Page)
                    .filter_by(slug=OrphanageService.ORPHANAGE_SLUG)
                    .first()
                )
                if orphanage:
            ...
            try:
                ...
                try:
                    ...
                    result = db.session.execute(
                        db.text(
                            "SELECT slug, file_path FROM pages WHERE id = CAST(:page_id AS uuid)"
                        ),
                        {"page_id": page_id_str},
                    ).first()
                    if not result:
                    ...
                    if page:
                        ...
                        new_file_path = FileService.calculate_file_path(
                            page, parent=orphanage
                        )
                        if old_file_path != new_file_path:
                            db.session.execute(
                                db.text(
                                    "UPDATE pages SET file_path = :file_path WHERE id = CAST(:page_id AS uuid)"
                                ),
                                {"file_path": new_file_path, "page_id": page_id_str},
                            )
                            db.session.commit()
                            try:
                                FileService.move_page_file(
                                    page, old_file_path, new_file_path
                                )
                            except Exception:
                                pass
    ...
    def get_orphaned_pages(grouped: bool = False) -> List[Page]:
        ...
        orphaned = (
            db.session.query(Page)
            .filter_by(is_orphaned=True)
            .order_by(Page.orphaned_from, Page.title)
            .all()
        )
    ...
    @staticmethod
    def reassign_page(
        page_id: uuid.UUID, new_parent_id: Optional[uuid.UUID], user_id: uuid.UUID
    ) -> Page:
        """
    ...
    def clear_orphanage(user_id: uuid.UUID) -> Dict:
        ...
        return {
            ...
            "pages": [
                {"id": str(p.id), "title": p.title, "slug": p.slug} for p in reassigned
            ],
        }
    ...
    def get_orphanage_stats() -> Dict:
        ...
        return {
            ...
            "by_parent": {
                parent_id: len(pages) for parent_id, pages in by_parent.items()
            },
        }
