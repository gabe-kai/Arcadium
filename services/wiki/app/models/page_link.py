...
class PageLink(db.Model):
    ...
    to_page_id = Column(UUID(as_uuid=True), ForeignKey('pages.id', ondelete='CASCADE'), nullable=False, index=True)
    link_text = Column(String(255), nullable=True)
    created_at = Column(DateTime, default=lambda: datetime.now(timezone.utc))
    
    # Relationships
    from_page = relationship('Page', foreign_keys=[from_page_id], backref='outgoing_links')
