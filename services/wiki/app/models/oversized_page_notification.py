...
from sqlalchemy import Column, Float, Boolean, ForeignKey, DateTime, JSON
...
class OversizedPageNotification(db.Model):
    ...
    notified_users = Column(JSON, nullable=False)  # Array of user IDs (stored as JSON for SQLite compatibility)
    ...
    def to_dict(self):
        ...
        return {
            ...
            'resolution_due_date': self.resolution_due_date.isoformat() if self.resolution_due_date else None,
            'notified_users': [str(uid) for uid in self.notified_users] if self.notified_users else [],
