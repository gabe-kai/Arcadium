...
def get_orphanage():
    ...
    try:
        ...
        for page in orphaned_pages:
            page_dict = {"id": str(page.id), "title": page.title, "slug": page.slug}
...
def reassign_orphaned_pages():
    ...
    try:
        ...
        reassigned = OrphanageService.bulk_reassign_pages(
            page_ids=page_uuids, new_parent_id=new_parent_id, user_id=user_id
        )
    ...
    try:
        ...
        if reassign_to_str:
            try:
                ...
                if not parent:
                    return (
                        jsonify({"error": f"Parent not found: {reassign_to_str}"}),
                        400,
                    )
            except ValueError:
        ...
        if reassign_to:
            ...
            reassigned = OrphanageService.bulk_reassign_pages(
                page_ids=page_ids, new_parent_id=reassign_to, user_id=user_id
            )
