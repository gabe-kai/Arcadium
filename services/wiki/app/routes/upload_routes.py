...
def upload_image():
    ...
    try:
        ...
        logging.info(
            f"Image saved: filename={filename}, full_path={full_path}, saved={file_saved}, size={file_size_saved}, expected_size={size_bytes}"
        )

        if not file_saved or file_size_saved != size_bytes:
            logging.error(
                f"File save verification failed: exists={file_saved}, size={file_size_saved}, expected={size_bytes}"
            )
...
def serve_image(filename):
    ...
    try:
        ...
        if not os.path.abspath(file_path).startswith(os.path.abspath(uploads_dir)):
            logging.error(
                f"Security violation: attempted to access file outside uploads directory: {file_path}"
            )
        ...
        if not os.path.exists(file_path):
            logging.error(
                f"Image file not found: {file_path} (uploads_dir: {uploads_dir}, filename: {filename})"
            )
        ...
        logging.info(
            f"Serving image: filename={filename}, file_path={file_path}, exists={file_exists}, size={file_size}, uploads_dir={uploads_dir}"
        )
        ...
        try:
            ...
            response = send_file(file_path_abs, mimetype=mime_type, as_attachment=False)
            ...
            logging.error(
                f"File path checked: {file_path_abs}, exists: {os.path.exists(file_path_abs)}"
            )
            return jsonify({"error": "Image not found"}), 404
        except Exception as send_error:
            logging.error(
                f"Error in send_file for {file_path_abs}: {str(send_error)}",
                exc_info=True,
            )
            raise
