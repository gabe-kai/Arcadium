...
class MarkdownFileHandler(FileSystemEventHandler):
    ...
    def __init__(self, sync_utility: SyncUtility, debounce_seconds: float = 1.0, app: Flask = None):
        """
    ...
    def _schedule_sync(self, file_path: str):
        ...
        with self.lock:
            ...
            self.timer = threading.Timer(self.debounce_seconds, self._sync_pending_files)
            self.timer.start()
    ...
    def _sync_file(self, file_path: str):
        ...
        if not self.app:
            print(f"[WATCHER] Error: No Flask app context available")
            return
...
class FileWatcher:
    ...
    def __init__(self, app: Flask, admin_user_id: Optional[str] = None, debounce_seconds: float = 1.0):
        """
