...
"""
        ...
        link = db.session.query(PageLink).filter_by(
...
def test_sync_file_updates_search_index(temp_pages_dir, app, sync_utility, admin_user_id):
...
"""
        ...
        index_entries = db.session.query(IndexEntry).filter_by(
            page_id=page.id
        ).all()
...
def test_sync_file_creates_version_on_update(temp_pages_dir, app, sync_utility, admin_user_id):
    ...
    with app.app_context():
        ...
        initial_versions = db.session.query(PageVersion).filter_by(page_id=page.id).all()
...
"""
        ...
        from app.models.page_version import PageVersion
        versions = db.session.query(PageVersion).filter_by(
            page_id=page.id
        ).all()
        ...
        synced_version = next((v for v in versions if v.change_summary == "Synced from file"), None)
        assert synced_version is not None
        assert synced_version.change_summary == "Synced from file"


def test_sync_file_no_version_on_create(temp_pages_dir, app, sync_utility, admin_user_id):
...
"""
        ...
        synced_versions = [v for v in versions if v.change_summary == "Synced from file"]
        assert len(synced_versions) == 0


def test_sync_file_with_multiple_links(temp_pages_dir, app, sync_utility, admin_user_id):
...
"""
        ...
        links = db.session.query(PageLink).filter_by(
            from_page_id=page.id
        ).all()
