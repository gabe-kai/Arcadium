...
def test_file_watcher_init():
    ...
    watcher = FileWatcher(app=mock_app, admin_user_id=None, debounce_seconds=2.0)
...
def test_file_watcher_start():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        with patch("app.sync.file_watcher.SyncUtility") as mock_sync_class, patch(
            "app.sync.file_watcher.Observer"
        ) as mock_observer_class, patch("os.path.abspath", return_value=tmpdir), patch(
