...
from app.sync.cli import sync_all_command, sync_file_command, sync_dir_command, main


def test_sync_all_command_success(capsys):
    """Test sync-all command success"""
    with patch('app.sync.cli.create_app') as mock_create_app, \
         patch('app.sync.cli.SyncUtility') as mock_sync_class:
...
def test_main_sync_all(capsys):
    ...
    with patch('sys.argv', ['sync', 'sync-all']), \
...
def test_main_sync_all_with_flags(capsys):
    ...
    with patch('sys.argv', ['sync', 'sync-all', '--force', '--admin-user-id', 'test-uuid']), \
         patch('app.sync.cli.sync_all_command') as mock_command:
...
def test_main_sync_file(capsys):
    ...
    with patch('sys.argv', ['sync', 'sync-file', 'test.md']), \
         patch('app.sync.cli.sync_file_command') as mock_command:
...
def test_main_sync_file_with_force(capsys):
    ...
    with patch('sys.argv', ['sync', 'sync-file', 'test.md', '--force']), \
         patch('app.sync.cli.sync_file_command') as mock_command:
...
def test_main_sync_dir(capsys):
    ...
    with patch('sys.argv', ['sync', 'sync-dir', 'section1']), \
         patch('app.sync.cli.sync_dir_command') as mock_command:
        
        main()
        mock_command.assert_called_once_with('section1', force=False, admin_user_id=None)
