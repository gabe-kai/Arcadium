...
def test_sync_file_parent_slug_not_found(temp_pages_dir, app, sync_utility, admin_user_id):
...
def test_sync_all_handles_errors(temp_pages_dir, app, sync_utility, admin_user_id):
    ...
    with app.app_context():
        ...
        with open(os.path.join(temp_pages_dir, 'no-slug.md'), 'w', encoding='utf-8') as f:
            f.write('---\ntitle: "No Slug"\n---\n\n# No Slug')
        
        # File that will cause error (invalid slug format)
        with open(os.path.join(temp_pages_dir, 'bad-slug.md'), 'w', encoding='utf-8') as f:
            f.write('---\ntitle: "Bad"\nslug: "bad slug"\n---\n\n# Bad')
...
def test_sync_directory_nested_subdirectories(temp_pages_dir, app, sync_utility, admin_user_id):
    """Test syncing directory includes nested subdirectories"""
    with app.app_context():
        # Create nested structure
        os.makedirs(os.path.join(temp_pages_dir, 'section1', 'subsection'), exist_ok=True)
        
        # File in section1 root
        with open(os.path.join(temp_pages_dir, 'section1', 'root.md'), 'w', encoding='utf-8') as f:
            f.write('---\ntitle: "Root"\nslug: "root"\n---\n\n# Root')
        
        # File in nested subsection
        with open(os.path.join(temp_pages_dir, 'section1', 'subsection', 'nested.md'), 'w', encoding='utf-8') as f:
            f.write('---\ntitle: "Nested"\nslug: "nested"\n---\n\n# Nested')
