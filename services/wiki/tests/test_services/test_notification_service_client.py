...
import pytest
import requests
from app.services.notification_service_client import NotificationServiceClient, get_notification_client
...
def test_send_notification_timeout():
    ...
    with patch('app.services.notification_service_client.requests.post', side_effect=requests.exceptions.Timeout()):
        result = client.send_notification(
            recipient_ids=['user-id'],
            subject='Test',
            content='Content'
...
def test_send_notification_connection_error():
    ...
    with patch('app.services.notification_service_client.requests.post', side_effect=requests.exceptions.ConnectionError()):
        result = client.send_notification(
            recipient_ids=['user-id'],
            subject='Test',
            content='Content'
