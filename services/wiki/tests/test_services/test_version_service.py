...
def test_create_multiple_versions(app):
    ...
    with app.app_context():
        ...
        version4 = VersionService.create_version(
            page_id=page.id,
            user_id=user_id
        )
...
def test_calculate_diff(app):
    ...
    with app.app_context():
        ...
        assert 'lines_added' in version2.diff_data or 'added_lines' in version2.diff_data
        assert 'lines_removed' in version2.diff_data or 'removed_lines' in version2.diff_data
        # Check that there were changes
        added = version2.diff_data.get('lines_added', version2.diff_data.get('added_lines', 0))
        removed = version2.diff_data.get('lines_removed', version2.diff_data.get('removed_lines', 0))
...
def test_compare_versions(app):
    ...
    with app.app_context():
        ...
        # Update page (PageService creates version 2)
        PageService.update_page(page_id=page.id, user_id=user_id, content="Version 2 content")
