...
def test_create_or_update_status_page_create(mock_check_all, app, test_user_id):
    ...
    with app.app_context():
        ...
        mock_check_all.return_value = {
            'wiki': {'status': 'healthy', 'response_time_ms': 5.0, 'error': None, 'details': {}},
            'auth': {'status': 'healthy', 'response_time_ms': 12.0, 'error': None, 'details': {}},
            'notification': {'status': 'degraded', 'response_time_ms': 250.0, 'error': 'High latency', 'details': {}}
        }
        
        page = ServiceStatusService.create_or_update_status_page(test_user_id, mock_check_all.return_value)
