...
def test_send_notification_400_bad_request():
    ...
    with patch(
        ...
        result = client.send_notification(
            recipient_ids=["invalid-id"], subject="Test", content="Content"
        )
...
def test_send_notification_empty_recipients():
    ...
    with patch(
        ...
        result = client.send_notification(
            recipient_ids=[], subject="Test", content="Content"
        )
        assert result is False
...
def test_send_notification_with_service_token():
    ...
    client = NotificationServiceClient(
        base_url="http://localhost:8006", service_token="test-service-token"
    )
...
def test_send_oversized_page_notification_multiple_pages():
    ...
    with patch(
        ...
        payload = call_args[1]["json"]
        assert (
            "10000.0" in payload["content"] or "9999.99" in payload["content"]
        )  # May be rounded
