...
def test_get_max_page_size_kb_configured(app, test_user_id):
    ...
    with app.app_context():
        # Set config
        config = WikiConfig(
            key="page_max_size_kb", value="500.0", updated_by=test_user_id
        )
...
def test_create_oversized_notifications(app, test_pages, test_user_id):
    ...
    with app.app_context():
        ...
        notifications = SizeMonitoringService.create_oversized_notifications(
            max_size_kb=100.0, resolution_due_date=due_date
        )
...
def test_create_oversized_notifications_updates_existing(app, test_pages, test_user_id):
    ...
    with app.app_context():
        ...
        SizeMonitoringService.create_oversized_notifications(
            max_size_kb=500.0, resolution_due_date=due_date2
        )
...
def test_check_and_resolve_oversized_pages(app, test_pages, test_user_id):
    ...
    with app.app_context():
        # Set limit to 100 KB
        config = WikiConfig(
            key="page_max_size_kb", value="100.0", updated_by=test_user_id
        )
        db.session.add(config)
        db.session.commit()
        ...
        notif = (
            ...
            .filter_by(page_id=large_page.id)
            .first()
        )
...
def test_create_oversized_notifications_with_user_ids(app, test_pages, test_user_id):
    ...
    with app.app_context():
        ...
        notifications = SizeMonitoringService.create_oversized_notifications(
            max_size_kb=100.0, resolution_due_date=due_date, user_ids=[custom_user_id]
        )
