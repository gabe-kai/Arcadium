...
def test_extract_selection_creates_link(client, app, test_writer_id):
    ...
    with mock_auth(test_writer_id, 'writer'):
        ...
        with app.app_context():
            ...
            forward_link = db.session.query(PageLink).filter_by(
            ...
            reverse_link = db.session.query(PageLink).filter_by(
    ...
    with mock_auth(test_writer_id, 'writer'):
        ...
        with app.app_context():
            ...
            for version in versions:
                if version.change_summary and 'extracted' in version.change_summary.lower():
...
def test_extract_selection_page_not_found(client, app, test_writer_id):
    ...
    with mock_auth(test_writer_id, 'writer'):
        response = client.post(f'/api/pages/{fake_id}/extract',
            json={
                'selection_start': 0,
                'selection_end': 10,
                'new_title': 'Test',
