...
from tests.test_api.conftest import mock_auth, auth_headers, test_user_id, test_admin_id
...
def test_reassign_orphaned_pages_actually_updates_page_state(client, app, test_user_id, test_admin_id):
    ...
    with mock_auth(test_admin_id, 'admin'):
        response = client.post('/api/orphanage/reassign',
            json={
                'page_ids': [orphan_id],
    ...
    with app.app_context():
        ...
        assert updated_page.is_orphaned == False, "Page should no longer be orphaned"
        assert updated_page.orphaned_from is None, "orphaned_from should be cleared"
        assert str(updated_page.parent_id) == new_parent_id, "parent_id should be updated"


def test_reassign_orphaned_pages_to_root_clears_parent(client, app, test_user_id, test_admin_id):
    ...
    with app.app_context():
        ...
        assert updated_page.is_orphaned == False
...
def test_reassign_non_orphaned_page_fails(client, app, test_user_id, test_admin_id):
    ...
    with mock_auth(test_admin_id, 'admin'):
        ...
        assert response.status_code == 200  # Route succeeds, but service skips non-orphaned pages
        data = response.get_json()
        assert data['reassigned'] == 0  # No pages reassigned


def test_reassign_orphaned_pages_circular_reference_prevention(client, app, test_user_id, test_admin_id):
    ...
    with mock_auth(test_admin_id, 'admin'):
        ...
        data = response.get_json()
        assert data['reassigned'] == 0  # But no pages reassigned due to circular reference


def test_reassign_orphaned_pages_remaining_count_accuracy(client, app, test_user_id, test_admin_id):
...
def test_clear_orphanage_actually_reassigns_pages(client, app, test_user_id, test_admin_id):
    ...
    with mock_auth(test_admin_id, 'admin'):
        response = client.post('/api/orphanage/clear',
            json={},
            headers=auth_headers(test_admin_id, 'admin')
        )
        assert response.status_code == 200
    ...
    with app.app_context():
        for orphan_id_str in orphan_ids:
            ...
            assert page.is_orphaned == False
            assert page.orphaned_from is None


def test_clear_orphanage_with_reassign_to_actually_reassigns(client, app, test_user_id, test_admin_id):
