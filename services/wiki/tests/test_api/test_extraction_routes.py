...
from tests.test_api.conftest import mock_auth, auth_headers, test_user_id, test_writer_id, test_admin_id


def test_extract_selection_requires_auth(client):
    """Test that extract selection requires authentication"""
    response = client.post(f'/api/pages/{uuid.uuid4()}/extract',
        json={
            'selection_start': 0,
            'selection_end': 10,
            'new_title': 'Test',
...
def test_extract_selection_requires_writer(client, app, test_user_id):
    ...
    with mock_auth(test_user_id, 'viewer'):
        response = client.post(f'/api/pages/{page_id}/extract',
            json={
                'selection_start': 0,
                'selection_end': 10,
                'new_title': 'Test',
...
def test_extract_heading_section_requires_auth(client):
    ...
    response = client.post(f'/api/pages/{uuid.uuid4()}/extract-heading',
        json={
...
def test_extract_heading_section_success(client, app, test_writer_id):
    ...
    with mock_auth(test_writer_id, 'writer'):
        response = client.post(f'/api/pages/{page_id}/extract-heading',
            json={
...
def test_promote_section_from_toc_requires_auth(client):
    ...
    response = client.post(f'/api/pages/{uuid.uuid4()}/promote-section',
        json={
...
def test_promote_section_from_toc_success(client, app, test_writer_id):
    ...
    with mock_auth(test_writer_id, 'writer'):
        response = client.post(f'/api/pages/{page_id}/promote-section',
            json={
                'heading_anchor': 'section-title',
...
def test_extract_selection_with_parent(client, app, test_writer_id):
    ...
    with mock_auth(test_writer_id, 'writer'):
        response = client.post(f'/api/pages/{source_id}/extract',
            json={
                'selection_start': 10,
                'selection_end': 25,
                'new_title': 'Extracted',
...
def test_extract_heading_promote_as_sibling(client, app, test_writer_id):
    ...
    with mock_auth(test_writer_id, 'writer'):
        response = client.post(f'/api/pages/{source_id}/extract-heading',
            json={
                'heading_text': 'Section',
                'heading_level': 2,
