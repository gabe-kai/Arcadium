...
def test_search_section_filter_nonexistent(client, app, test_page, test_user_id):
    ...
    for result in data['results']:
        assert result.get('section') == 'nonexistent-section'  # Should be empty if filter works


def test_search_include_drafts_writer_other_user(client, app, test_user_id, test_writer_id):
    ...
    with app.app_context():
        ...
        db.session.commit()
        SearchIndexService.index_page(draft_page.id, draft_page.content, draft_page.title)
    
    with mock_auth(test_writer_id, 'writer'):
        # Writer tries to see other user's draft
        response = client.get('/api/search?q=draft&include_drafts=true',
...
def test_get_master_index_empty(client):
    ...
    assert len(data['index']) == 0 or all(len(pages) == 0 for pages in data['index'].values())
