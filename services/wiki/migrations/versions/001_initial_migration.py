...
def upgrade():
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['orphaned_from'], ['pages.id'], ),
    ...
    # Partial indexes
    op.execute("CREATE INDEX idx_pages_orphaned ON pages(is_orphaned) WHERE is_orphaned = TRUE")
    op.execute("CREATE INDEX idx_pages_system ON pages(is_system_page) WHERE is_system_page = TRUE")
    ...
    # Partial indexes
    op.execute("CREATE INDEX idx_index_keyword ON index_entries(is_keyword) WHERE is_keyword = TRUE")
    op.execute("CREATE INDEX idx_index_manual ON index_entries(is_manual) WHERE is_manual = TRUE")
    
    # Create indexes for page_versions table
    op.create_index('idx_versions_page', 'page_versions', ['page_id'])
    op.create_index('idx_versions_version', 'page_versions', ['page_id', 'version'], postgresql_ops={'version': 'DESC'})
    ...
    op.create_index('idx_oversized_resolved', 'oversized_page_notifications', ['resolved'])
    op.create_index('idx_oversized_due_date', 'oversized_page_notifications', ['resolution_due_date'])
